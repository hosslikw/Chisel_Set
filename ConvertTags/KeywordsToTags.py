...
# Keywords to Tags
# Many of the useful keywording tools save the keywords in the XMP metadata of the file. Mac OS doesn't preserve these keywords. The script below will add tags to each file based on the keywords in the XMP metadata. Your tagging should now be accurate in the Finder and in third party applications.
...
def keyword_log(directory_path):
    for root, _, files in os.walk(directory_path):
        for file in files:
            ...
            try:
                raw_keywords = subprocess.check_output(["exiftool", "-XMP:Subject", "-b", input_file_path], stderr=subprocess.STDOUT).decode().strip()  # noqa
